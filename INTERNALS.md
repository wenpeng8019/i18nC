...
不使用 `-P`（保留 `#line` 指令），以便中间 awk 过滤器区分来自 inline header 展开的行。  
`-include` 标志注入临时头文件，不修改任何源文件。

预处理输出经过一层**文件过滤 awk**后再交给字符串提取器：

```awk
# 解析 # <line> "<file>" 指令，只保留属于目标源文件的行
/^#/ { if (match($0, /"[^"]+"/) > 0) cur = substr($0, RSTART+1, RLENGTH-2); next }
cur == target || cur == "" { print }
```

这一步过滤掉 `#include` 展开引入的 inline 函数代码，
避免头文件中的 `LA_*` 调用污染源文件的行号 / occ 计数。
详见下方"特殊情况处理 → 8. inline header 展开行号碰撞"。
...
- MSVC 预处理器：主扫描用 `/E`（保留 `#line`）+ `FilterTarget` 过滤 inline header 行；
  探测阶段用 `/EP`（无 `#line`）。`/FI<file>` 强制包含 marker 头文件
- GCC/Clang：主扫描用 `-E`（保留 `#line`）+ `FilterTarget` 过滤；
  探测阶段用 `-E -P`。`-include` 注入 marker 头文件
...
### 8. inline header 展开行号碰撞

`cc -E` 展开 `#include` 时，头文件中的 `static inline` 函数会被完整复制到
预处理输出中。如果 inline 函数内含有 `LA_F` 调用，其 `_I18NLINE_ __LINE__`
取的是**头文件定义处的行号**，而非源文件行号。

**碰撞条件**：头文件某行号 == 源文件某行号，且两行都包含 `LA_*` 调用。

**典型场景**：

```c
// p2p_internal.h:428 — inline 函数
static inline int reserve() {
    ... LA_F("Failed to realloc memory...", LA_F387, 387) ...
}

// p2p_signal_compact.c:428 — 源码
printf(LA_F("[UDP] Resend %s to %s:%d...", LA_F362, 362), ...);
```

compact.c `#include` internal.h 后，`cc -E` 输出中 line 428 出现两个 `_I18NF_` 标记：
一个来自 header（realloc, occ=1），一个来自源文件（Resend, occ=2）。
但源文件第 428 行只有**一个** `LA_F`（occ=1），回写时 occ=1 映射到了
header 的 realloc → 源文件被错误改为 `LA_F387`。

**解决方案**（已修复）：

阶段 1 不使用 `-P`（保留 `#line` 指令），在预处理输出和字符串提取器之间
插入一层 awk 过滤器，解析 `# <line> "file"` 指令追踪当前文件，
只输出属于目标源文件的行。这样 header 展开的 inline 函数被完全过滤。

**阶段 0.5 不受影响**：`#define` 探测使用独立的 `_probe_tmp` 文件编译，
其 `#include` 原文件虽然也会展开 header，但探测阶段仍使用 `-E -P`，
且 `#line` 指令将行号设为原始定义行号，不存在碰撞问题。

### 9. LANG.SUFFIX.h 引用过期（disabled / hole 切换）

`.LANG.h` 中 disabled 条目的枚举名为 `LA_F{SID}`（如 `LA_F175`），
而完全删除的空洞用 `_LA_{SID}` 占位（如 `_LA_175`）。

当一个条目在不同 i18n 运行中在"空洞"和"disabled"之间切换时，
枚举名会从 `_LA_175` 变为 `LA_F175`（或反向）。
如果此时 `LANG.SUFFIX.h`（如 `LANG.cn.h`）未随 `--import` 重新生成，
其中的 `[_LA_175]` 引用已不存在的枚举常量，导致编译错误。

**触发条件**：
1. 某次运行中条目消失，变为空洞 `_LA_N`
2. 后续运行中旧 `.LANG.c` 的 `TEMP_OLD_SID_MAP` 恢复了该 SID，变为 disabled `LA_F{SID}`
3. 构建时未开启 `--import SUFFIX`，`LANG.SUFFIX.h` 未更新

**规避方式**：修改 i18n 配置后，确保开启 `--import` 重新生成翻译文件；
或在 CMake 中始终启用 `I18N_CN`。

### 10. BSD mktemp 模板后缀

`mktemp /tmp/xxx_XXXXXX.c` 在 macOS（BSD mktemp）上不合法——模板的
`X` 占位符必须位于末尾，后面不能有 `.c` 等后缀。BSD mktemp 会把字面量
`XXXXXX.c` 当作固定文件名，导致：

- 第一次调用创建 `/tmp/xxx_XXXXXX.c`（无随机化）
- 第二次调用（并行构建或同目录循环）失败：`mkstemp: File exists`

**解决方案**（已修复）：去掉 `.c` 后缀，使用 `mktemp /tmp/xxx_XXXXXX`。
Phase 0.5 探测编译已显式传入 `-x c`，无需依赖文件扩展名。

### 11. Windows MSVC .h 文件 `#line` 路径斜杠不匹配

PowerShell 版本（`i18n.ps1`）处理 `.h` 文件时，会创建一个临时 `.c` 包装，
其中 `#include` 的路径被 `Replace('\', '/')` 转为正斜杠。
MSVC `/E` 输出的 `#line` 指令会引用该正斜杠路径，但 `$FilterTarget`
保持原始的 Windows 反斜杠路径，导致 `$cur -eq $FilterTarget` 不匹配，
inline header 过滤器失效（所有行被丢弃）。

**解决方案**（已修复）：比较前将两侧路径统一转为正斜杠：
```powershell
